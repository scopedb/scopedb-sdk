...
impl<T: DeserializeOwned> Response<T> {
    pub(crate) async fn from_http_response(r: reqwest::Response) -> Result<Self, Error> {
        ...
        if let Ok(resp) = serde_json::from_slice::<ErrorContext>(&payload) {
            return Ok(Response::Failure(resp));
        }

        Ok(Response::Error(ErrorStatus {
            code: status,
...
pub struct StatementRequest {
    #[serde(default = "default_random_statement_id")]
...
fn default_random_statement_id() -> Uuid {
    Uuid::new_v4()
}
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct StatementFailureResponse {
    pub statement_id: Uuid,
...
pub struct StatementResponse {
    ...
    pub result_set: Option<StatementResultSet>,
}
